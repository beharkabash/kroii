...
export class SEOGenerator {
  ...
  static generateCarJSONLD(car: CarSEOData) {
    return {
      ...
      "description": car.description || `${car.make} ${car.model} ${car.year} - ${car.kmNumber.toLocaleString('fi-FI')} km`,
      "brand": {
        "@type": "Brand",
        "name": car.make
      },
  ...
  static generateCarMetadata(car: CarSEOData): Metadata {
    const title = `${car.make} ${car.model} ${car.year} - ${car.priceEur.toLocaleString('fi-FI')}€ | Kroi Auto Center`;
    const description = `${car.make} ${car.model} ${car.year}, ${car.kmNumber.toLocaleString('fi-FI')} km, ${car.fuel}, ${car.transmission}. Hinta ${car.priceEur.toLocaleString('fi-FI')}€. Laadukkaita käytettyjä autoja Helsingissä.`;
    const imageUrl = car.images[0] ? `${this.baseUrl}${car.images[0]}` : `${this.baseUrl}/og-car-default.jpg`;

    return {
      title,
      description,
      keywords: `${car.make}, ${car.model}, ${car.year}, käytetty auto, autokauppa Helsinki, ${car.fuel}, ${car.transmission}`,
      openGraph: {
        title,
        description,
        type: 'website',
